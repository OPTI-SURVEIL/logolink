...
```{r}
devtools::install_github('pcollender/ChinSimi')
library(ChinSimi) #https://github.com/canalcheng/ChinSimi - functions for Chinese character conversion to pinyin and FCC and string similarity calculations
...
source('Linkage Models/corlink2.R') #set to path where you have stored the corlink2.R file. This contains elements of a package to estimate probabilities of matches between records based on their pattern of agreement across fields, and impute missing agreement patterns.
...
for(c in cols_to_link){
  ...
  if(length(col1)>1){
    ...
    allvals = na.omit(unique(c(S1[,col1],S2[,col2])))
...
for(c in cols_to_link){
  if(c %in% sim_vars){
    ...
    tempm = unique(rbind(expand.grid(m_inds[[c]][[1]],1:nrow(S2)),expand.grid(1:nrow(S1),m_inds[[c]][[2]])))
    
  }else{
    temp = do.call(rbind,lapply(u_inds[[c]], expand.grid)) #store coordinates of every exact match
    tempm = unique(rbind(expand.grid(m_inds[[c]][[1]],1:nrow(S2)),expand.grid(1:nrow(S1),m_inds[[c]][[2]])))
  }
  SpMats[[c]] = sparseMatrix(i = c(temp[,1],tempm[,1]), 
                               j = c(temp[,2],tempm[,2]),
                               x = c(rep(1,nrow(temp)), rep(999, nrow(tempm))),
                               dims = c(nrow=nrow(S1), ncol = nrow(S2)))
}
rm(list = c('temp','tempm','uvalnames'))
gc()
```

##5. Generate a frequency table of agreement patterns for all fields

####And save indices of record pairs that could potentially be linked (i.e. match on either first name or last name, as well as at least 2 of sex, yob, mob, dob)
```{r}
...
slices = lapply(seq_len(length(todo) %/% 1e6 + 1),
                function(i) (1e6 * i - 1e6 + 1):pmin(1e6*i,length(todo)))
...
patlist = foreach(i = 1:length(slices)) %dopar% {
  ...
  pats = unique(pat)
  pats = apply(pats,1,paste0,collapse='')
  pats_ = apply(pat,1,paste0,collapse='')
  plist = vector('list',length(pats))
  names(plist) = pats
  
  for(i in 1:length(pats)){
    pinds = which(pats_==pats[i])
    plist[[pats[i]]] = cbind(i_s[pinds],j_s[pinds])
  }
...
mastertable = do.call(rbind,lapply(patlist,'[[',1)) #contingency table of frequency of each observed agreement pattern
savepats = unique(unlist(lapply(lapply(patlist,'[[',2),names)))
patt_inds = lapply(savepats, function(p) do.call(rbind,lapply(patlist,function(l) l[[2]][[p]]))) #named list of agreement patterns with stored indices, i.e. those that are possible matches a priori

names(patt_inds) = savepats
...
allpats = rep(list(c(0,1)),length(cols_to_link))
names(allpats) = cols_to_link
for(c in cols_to_link){ #filling in remaining patterns in contingency table
  if(max(unlist(sapply(m_inds[[c]],length))) > 0) allpats[[c]] = c(allpats[[c]],999)
}

allpats = expand.grid(allpats)
allpats_ = apply(allpats,1,paste0,collapse='')

allpats = allpats[-which(allpats_ %in% savepats),]

newcounts = foreach(p = 1:nrow(allpats),.combine='c') %dopar% { #this step takes a while
    
    v = unlist(allpats[p,])
    cands = which(SpMats[[1]]==v[1])
    for(m in 2:length(SpMats)){
      cands = intersect(cands, which(SpMats[[m]] == v[m]))
    }
    length(cands)
  }
  
newfreqs = cbind(allpats,newcounts)
colnames(newfreqs) = colnames(mastertable)
mastertable = rbind(data.frame(mastertable),newfreqs)
mastertable = subset(mastertable,counts>0)
mastertable[mastertable==999] = NA

stopCluster(cl)

rm(list=c('newfreqs', 'newcounts', 'allpats_', 'allpats', 'savepats', 'patlist', 'slices', 'suff_cols', 'part_cols','todo'))
...
```{r}
plot(mcorr(ifit$fitted_models,ifit$imputed_freqs)-bcorr(ifit$imputed_freqs),ylim=c(-.25,.25), type='l',main='independence',ylab = 'correlation error', xlab = '')
text(x = 1:length(bcorr(ifit$imputed_freqs)), y = mcorr(ifit$fitted_models,ifit$imputed_freqs)-bcorr(ifit$imputed_freqs) +  0.1,labels= names(bcorr(ifit$imputed_freqs)), cex = .8, srt = 60)
plot(mcorr(mfit$fitted_models,mfit$imputed_freqs)-bcorr(mfit$imputed_freqs),ylim=c(-.25,.25), type='l',main='match interactions',ylab = 'correlation error', xlab = '')
text(x = 1:length(bcorr(ifit$imputed_freqs)), y = mcorr(mfit$fitted_models,ifit$imputed_freqs)-bcorr(ifit$imputed_freqs) +  0.1,labels= names(bcorr(ifit$imputed_freqs)), cex = .8, srt = 60)
plot(mcorr(bfit$fitted_models,bfit$imputed_freqs)-bcorr(bfit$imputed_freqs),ylim=c(-.25,.25), type='l',main='all interactions',ylab = 'correlation error', xlab = '')
text(x = 1:length(bcorr(ifit$imputed_freqs)), y = mcorr(bfit$fitted_models,ifit$imputed_freqs)-bcorr(ifit$imputed_freqs) +  0.1,labels= names(bcorr(ifit$imputed_freqs)), cex = .8, srt = 60)
```
...
```{r}
whichpats = subset(freqtable, fitted_prob_match >= thr.high)[1:length(cols_to_link)]
whichpats[is.na(whichpats)] = 999
whichpats = apply(whichpats,1,paste0,collapse='')

link_inds = do.call(rbind,lapply(whichpats,function(p) patt_inds[[p]]))

linked_db = cbind(S1[link_inds[,1], ], S2[link_inds[,2],])
linked_db = linked_db[,order(colnames(linked_db))]
colnames(linked_db) = paste0(rep(c('S1.','S2.'),ncol(S1)),colnames(linked_db))
colnames(linked_db) = gsub('.1$','',colnames(linked_db))
